...
class UpdateCompanyRequest extends FormRequest
{
    ...
    public function authorize(): bool
    {
        ...
        // Si c'est la route pour modifier sa propre company (/api/v1/company)
        if (!$this->route('company')) {
            return true; // L'utilisateur peut toujours modifier sa propre company
        }
        
        // Si c'est la route pour modifier une company spécifique (/api/v1/companies/{id})
        $companyId = $this->route('company');
        
        // Si l'utilisateur est super admin (à implémenter selon vos besoins)
        // if ($user->hasRole('super_admin')) {
        //     return true;
        // }
        
        // Si l'utilisateur appartient à cette company
    ...
    {
        // Si c'est la route /api/v1/company, utiliser l'ID de la company de l'utilisateur
        ...
        return [
            ...
            'siren' => [
                ...
                'size:9', 
                'regex:/^[0-9]{9}$/', 
                Rule::unique('companies', 'siren')->ignore($companyId)
            ],
            'siret' => [
                'nullable', 
                'string', 
                'size:14', 
                'regex:/^[0-9]{14}$/', 
                Rule::unique('companies', 'siret')->ignore($companyId)
            ],
