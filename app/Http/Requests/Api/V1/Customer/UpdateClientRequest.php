...
class UpdateClientRequest extends FormRequest
{
    ...
    public function authorize(): bool
    {
        ...
        if (!$client || !$this->user()->company) {
            return false;
        }

        // Check if client belongs to user's company
        if ($client->company_id !== $this->user()->company->id) {
            return false;
        }
    ...
    public function rules(): array
    {
        ...
        return [
            'client_type' => ['sometimes', 'string', Rule::in(['company', 'individual'])],
            ...
            'phone_numbers.*.id' => ['nullable', 'uuid'],
